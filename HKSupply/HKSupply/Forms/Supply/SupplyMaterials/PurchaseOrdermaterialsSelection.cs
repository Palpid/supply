...
{
    public partial class PurchaseOrderMaterialsSelection : RibbonFormBase
    {
        #region Colors
        ...
        #region Constructor
        public PurchaseOrderMaterialsSelection()
        {
            InitializeComponent();
        ...
        #region Form events
        private void PurchaseOrdermaterialsSelection_Load(object sender, EventArgs e)
        {
        ...
        private void LoadAuxList()
        {
            try
            {
                _suppliersList = GlobalSetting.SupplierService.GetSuppliers(withEtniaHk: true).Where(a => a.Factory == false).ToList();
        ...
        private void SetUpTabs()
        {
            try
            {
                xtpPOSection.AutoScroll = true;
                xtpPOSection.AutoScrollMargin = new Size(20, 20);
                xtpPOSection.AutoScrollMinSize = new Size(xtpPOSection.Width, xtpPOSection.Height);
            }
            catch
            {
                throw;
        ...
        private void OpenPoForm(POSelection po)
        {
            try
            {
                ...
                PurchaseOrderMaterials purchaseOrderMaterialsForm =
                    Application.OpenForms.OfType<PurchaseOrderMaterials>()
                    .Where(pre => pre.Name == "PurchaseOrderMaterials")
                    .SingleOrDefault();

                if (purchaseOrderMaterialsForm != null)
                    purchaseOrderMaterialsForm.Close();

                purchaseOrderMaterialsForm = new PurchaseOrderMaterials();
                purchaseOrderMaterialsForm.InitData(po.PoNumber);
